...
int rkispp_register_stream_vdevs(struct rkispp_device *dev)
{
	...
	if (dev->ispp_ver == ISPP_V10) {
		...
		hrtimer_setup(&stream_vdev->fec_qst, rkispp_fec_do_early, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		hrtimer_setup(&stream_vdev->frame_qst, stream_vdev->stream_ops->rkispp_frame_done_early, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
